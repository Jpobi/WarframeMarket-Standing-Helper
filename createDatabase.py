...
with open('mod_prices.csv', newline='', encoding='utf-8') as csvfile:
    reader = csv.reader(csvfile)
    next(reader)  # Skip header row
    for row in reader:
        mod_name, faction_name, price48hs, price90d = row
        formatted_name = format_string(mod_name)
        cursor.execute('''INSERT OR IGNORE INTO Mod (name, url_name, Price48hs, Price90d) VALUES (?, ?, ?, ?)''', (mod_name, formatted_name, price48hs, price90d))
